...
PACKAGE_DIRECTORY = package_directory = os_path.dirname(os_path.abspath(__file__))
...
class ImagesMatricielles:
    ...
    loaded = dict()
    chemin_racine = os_path.join(PACKAGE_DIRECTORY, 'images')
...
class IntegratorOutputError:
    ...
    def initialiser_gestion_erreurs():
        if mpl.get_backend() == 'nbAgg':
            ...
            with open(os_path.join(PACKAGE_DIRECTORY,'log_erreur.out'), 'w') as error_file:
                error_file.write('')
            logging_basicConfig(filename=os_path.join(PACKAGE_DIRECTORY,'log_erreur.out'), filemode = 'w')
...
class ExceptionManager:
    ...
    def manage_exception():
        ...
        if mpl.get_backend() == 'TkAgg':
            ...
            logging_exception('')
            with open(os_path.join(PACKAGE_DIRECTORY,'log_erreur.out'), 'r', newline = '') as error_file:
...
class Vue:
    ...
    def __init__(self, nb_lignes, nb_colonnes, max_dim = 7):
        ...
        - max_dim     : dimension maximale de la figure en 
                        inches (à 100 dpi)
        |
        |
        |
    ...
    def modifier_grille(self, objet, lig, col, **modifications):
        '''
        Permet de modifier les attributs d'un objet graphique présent 
        dans la grille.
        lig et col désignent les indices de ligne et de colonne de 
        l'objet (0, 0 : coin supérieur gauche).
            - objet : 'mur_v', 'mur_h', 'fond', 'texte', 'image'
            - lig : entier entre 0 et NB_LIGNES - 1 (sauf pour objet = 'mur_h' : 
                    entre 0 et NB_LIGNES)
            - col : entier entre 0 et NB_COLONNES - 1 (sauf pour objet = 'mur_v' : 
            entre 0 et NB_COLONNES)
            - **modifications :
               - objet 'mur_v':
                    - couleur_int ou ci : 3 ou 4-uplet d'entiers entre 0 et 255 au 
                                          format (R, G, B) ou (R, G, B, A) 
                                          
                    - couleur_ext ou ce : même type que couleur_int
                    
                    - couleur ou c      : même type que couleur_int, modifie simultanément 
                                          couleur_int et couleur_ext
               ...
               - objet 'texte':
                    - texte ou t          : chaîne de caractères (au delà de 3 caractères, 
                                            risque de débord)
                                            
                    - couleur ou c        : 3 ou 4-uplet d'entiers entre 0 et 255 au format 
                                            (R, G, B) ou (R, G, B, A)
                                            
                    - taille_police ou tp : entier, à ajuster (gestion du texte sous matplotlib
                                            est peu développée)
                                            
                    - visible ou v        : booléen indiquant si l'objet est visible ou pas
                    
                    - alpha ou a          : transparence, flottant entre 0 (transparent) et 1 
                                            (opaque)                    

               - objet 'image':
                    - chemin_fichier ou cf : chemin d'accès au fichier exprimé depuis la racine 
                                             de './images/'. Le fichier doit :
                                                - être au format .png
                                                - être situé dans le répertoire './images/' 
                                                  ou dans ses sous-répertoires
                                             Par défaut, initialisée avec './images/init.png' qui doit 
                                             donc exister.
                                             
                    - taille ou t          : flottant entre 0 et 1. Lorsque c'est inférieur à 1 
                                             un redimensionnement est effectué ce qui introduit du flou.
    ...
    def modifier_zonetexte(self, **modifications):
        ...
        Permet de modifier les attributs de la zone de texte.
            - **modifications :
                ...
                - couleur_texte ou ct : 3 ou 4-uplet d'entiers entre 0 et 255 au format 
                                        (R, G, B) ou (R, G, B, A)
                                        
                - couleur_fond ou cf  : 3 ou 4-uplet d'entiers entre 0 et 255 au format 
                                        (R, G, B) ou (R, G, B, A)
    ...
    def lier_evenement(self, evenement, gestionnaire, activer, argument = None):
        ...
        - evenement     : 'appui_touche', 'fin_appui_touche', 'selection_objet', 
                          'clic_souris' , 'fin_clic_souris
                          
        - gestionnaire  : fonction qui réceptionnera les évènements déclenchés
                          cette fonction doit avoir un paramètre obligatoire et 
                          un paramètre facultatif :
                           - e à qui sera affecté l'évènement qui a été déclenché 
                             (requis)
                           - a à qui sera affecté argument s'il a été fourni 
                             (facultatif)
                             
        - activer       : booléen indiquant si on active l'écoute ou désactive 
                          une écoute mise en place
        
        Voici les contenus des cinq évènements disponibles qui sont des dictionnaires 
        dont les clefs sont :
        ...
        Lors de l'activation de la mise en écoute il n'est pas possible d'écouter une 
        seule touche ou un seul objet de la grille. Le filtrage doit être effectué 
        en aval, dans la fonction ecouteur.
        
        En cas de superposition d'objets graphiques, l'évènement 'selection_objet' n'est 
        lancé que par celui ayant le zorder le plus élevé avec :
        - fond   : zorder = 0
        - image  : zorder = 1
        - mur_h  : zorder = 2
        - mur_v  : zorder = 3
        ...
        L'argument à transférer a vocation à être un tableau mutable (typiquement une 
        matrice ou un tableau de matrices)
        ...
        Elle n'est affectée que dans le cas des back-ends 'TkAgg' (tkinter) et 'nbAgg' 
        (notebook backend).
        |
        |
        |
